...
__declspec(noalias safebuffers) __forceinline HANDLE InlineCurrentProcessPseudohandle()noexcept{return{reinterpret_cast<HANDLE>(static_cast<std::intptr_t>(-1))};}
__declspec(noalias safebuffers) __forceinline HANDLE InlineCurrentThreadPseudohandle()noexcept{return{reinterpret_cast<HANDLE>(static_cast<std::intptr_t>(-2))};}
__declspec(noalias safebuffers) __forceinline HANDLE InlineCurrentSessionPseudohandle()noexcept{return{reinterpret_cast<HANDLE>(static_cast<std::intptr_t>(-3))};}

__declspec(noalias safebuffers) __forceinline void *InlineDerefTEBStackBasePtr()noexcept{
#ifdef _WIN64
	std::uintptr_t pStackBase{__readgsqword(0x8)};
#else
	std::uintptr_t pStackBase{__readfsdword(0x4)};
#endif
	assert(pStackBase);
...
#ifdef _WIN64
	std::uintptr_t pStackLimit{__readgsqword(0x10)};
#else
	std::uintptr_t pStackLimit{__readfsdword(0x8)};
#endif
	assert(pStackLimit);
	return{reinterpret_cast<void *>(pStackLimit)};
}

__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefTEBProcessId()noexcept{
	// GetCurrentProcessId() internal
#ifdef _WIN64
	std::uint32_t u32ProcessId{__readgsdword(0x40)};
#else
	std::uint32_t u32ProcessId{__readfsdword(0x20)};
#endif
	assert(u32ProcessId);
	return{u32ProcessId};
}

__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefTEBThreadId()noexcept{
	// GetCurrentThreadId() internal
#ifdef _WIN64
	std::uint32_t u32ThreadId{__readgsdword(0x48)};
#else
	std::uint32_t u32ThreadId{__readfsdword(0x24)};
#endif
	assert(u32ThreadId);
	return{u32ThreadId};
}
...
__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefTEBLastError()noexcept{
	// GetLastError() internal
#ifdef _WIN64
	std::uint32_t u32LastError{__readgsdword(0x68)};
#else
	std::uint32_t u32LastError{__readfsdword(0x34)};
#endif
	return{u32LastError};
}
...
#ifdef _WIN64
	std::uint32_t u32CurrentLocale{__readgsdword(0x108)};
#else
	std::uint32_t u32CurrentLocale{__readfsdword(0xC4)};
#endif
...
// For both dll and exe files various resouces can be extracted from this header. code: IMAGE_NT_HEADERS const *pNTHeader{reinterpret_cast<IMAGE_NT_HEADERS const *>(reinterpret_cast<std::uintptr_t>(pImageBaseAddress) + static_cast<std::size_t>(static_cast<ULONG>(pImageBaseAddress->e_lfanew)))};
...
#ifdef _WIN64
	IMAGE_DOS_HEADER *pImageBaseAddress{*reinterpret_cast<IMAGE_DOS_HEADER **>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x10)};// GetModuleHandle(nullptr) internal
#else
	IMAGE_DOS_HEADER *pImageBaseAddress{*reinterpret_cast<IMAGE_DOS_HEADER **>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x8)};// GetModuleHandle(nullptr) internal
#endif
...
#ifdef _WIN64
	HANDLE hProcessHeap{*reinterpret_cast<HANDLE *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x30)};
#else
	HANDLE hProcessHeap{*reinterpret_cast<HANDLE *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x18)};
#endif
	assert(hProcessHeap);
	return{hProcessHeap};
}

__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefPEBNumberOfProcessors(PEB *pProcessEnvironmentBlock)noexcept{
	// GetSystemInfo(&) .dwNumberOfProcessors internal
#ifdef _WIN64
	std::uint32_t u32NumberOfProcessors{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xB8)};
#else
	std::uint32_t u32NumberOfProcessors{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x64)};
#endif
...
#ifdef _WIN64
	std::uint32_t u32NtGlobalFlag{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xBC)};
#else
	std::uint32_t u32NtGlobalFlag{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x68)};
#endif
	return{u32NtGlobalFlag == 0x70 || pProcessEnvironmentBlock->BeingDebugged};
}

__declspec(noalias safebuffers) __forceinline std::uint8_t InlineDerefPEBOSMajorVersion(PEB *pProcessEnvironmentBlock)noexcept{
	// GetVersionEx(&) .dwMajorVersion internal
#ifdef _WIN64
	std::uint8_t u8OSMajorVersion{*reinterpret_cast<std::uint8_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x118)};
#else
	std::uint8_t u8OSMajorVersion{*reinterpret_cast<std::uint8_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xA4)};
#endif
...
__declspec(noalias safebuffers) __forceinline std::uint8_t InlineDerefPEBOSMinorVersion(PEB *pProcessEnvironmentBlock)noexcept{
	// GetVersionEx(&) .dwMinorVersion internal
#ifdef _WIN64
	std::uint8_t u8OSMinorVersion{*reinterpret_cast<std::uint8_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x11C)};
#else
	std::uint8_t u8OSMinorVersion{*reinterpret_cast<std::uint8_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xA8)};
#endif
...
__declspec(noalias safebuffers) __forceinline std::uint16_t InlineDerefPEBOSBuildNumber(PEB *pProcessEnvironmentBlock)noexcept{
	// GetVersionEx(&) .dwBuildNumber internal
#ifdef _WIN64
	std::uint16_t u16OSBuildNumber{*reinterpret_cast<std::uint16_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x120)};
#else
	std::uint16_t u16OSBuildNumber{*reinterpret_cast<std::uint16_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xAC)};
#endif
	assert(u16OSBuildNumber);
	return{u16OSBuildNumber};
}

__declspec(noalias safebuffers) __forceinline std::uint16_t InlineDerefPEBOSOSCSDVersion(PEB *pProcessEnvironmentBlock)noexcept{
	// GetVersionEx(&) .wServicePackMajor internal
#ifdef _WIN64
	std::uint16_t u16OSCSDVersion{*reinterpret_cast<std::uint16_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x120)};
#else
	std::uint16_t u16OSCSDVersion{*reinterpret_cast<std::uint16_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xAC)};
#endif
	assert(u16OSCSDVersion);
	return{u16OSCSDVersion};
}

__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefPEBOSPlatformId(PEB *pProcessEnvironmentBlock)noexcept{
	// GetVersionEx(&) .dwPlatformId internal
#ifdef _WIN64
	std::uint32_t u32OSPlatformId{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x124)};
#else
	std::uint32_t u32OSPlatformId{*reinterpret_cast<std::uint32_t *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0xB0)};
#endif
	return{u32OSPlatformId};
}

__declspec(noalias safebuffers) __forceinline std::uint32_t InlineDerefPEBSessionId(PEB *pProcessEnvironmentBlock)noexcept{
	// ProcessIdToSessionId(GetCurrentProcessId(), &) internal
	std::uint32_t u32SessionId{pProcessEnvironmentBlock->SessionId};
	assert(u32SessionId);
	return{u32SessionId};
}
...
#ifdef _WIN64
	UNICODE_STRING *pCSDVersion{reinterpret_cast<UNICODE_STRING *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x2E8)};
#else
	UNICODE_STRING *pCSDVersion{reinterpret_cast<UNICODE_STRING *>(reinterpret_cast<std::uintptr_t>(pProcessEnvironmentBlock) + 0x1F0)};
#endif
	return{pCSDVersion};
}
...
#ifdef _WIN64
	UNICODE_STRING *pDesktopInfo{reinterpret_cast<UNICODE_STRING *>(reinterpret_cast<std::uintptr_t>(pUserProcessParameters) + 0xC0)};
#else
	UNICODE_STRING *pDesktopInfo{reinterpret_cast<UNICODE_STRING *>(reinterpret_cast<std::uintptr_t>(pUserProcessParameters) + 0x78)};
#endif
	return{pDesktopInfo};
}